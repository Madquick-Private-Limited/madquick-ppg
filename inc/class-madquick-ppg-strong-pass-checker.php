...
class Madquick_ppg_strong_pass_checker {
    ...
    public static function init() {
        self::$settings = (array) get_option( 'madquick_ppg_settings', [] );
    ...
    public static function check_on_register( $user_id ) {
        // Reading a core wp-login.php field; we are not persisting/changing any global state from this input.
        // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Core registration form; only reading the password to advise strength.
        $pw_raw = filter_input( INPUT_POST, 'user_pass', FILTER_UNSAFE_RAW );
        $pw     = is_string( $pw_raw ) ? (string) wp_unslash( $pw_raw ) : '';
    ...
    public static function maybe_admin_notice() {
        ...
        $strong_password_keyword = isset( self::$settings['strong_password_generator_keyword'] ) && self::$settings['strong_password_generator_keyword'] !== ''
            ? self::$settings['strong_password_generator_keyword']
            : 'Strong password generator';

            $message = sprintf(
                // translators: 1: opening <a> tag, 2: anchor text, 3: closing </a>.
                __( 'You can generate a strong password at %1$s%2$s%3$s.', 'madquick-ppg' ),
    ...
    public static function login_ssr_info_notice() {
        ...
        $action     = $action_raw ? sanitize_key( $action_raw ) : 'login';
        ...
        // Build the link markup (not translatable, but safely escaped).
        $link_html = '<a href="' . esc_url( self::GEN_URL ) . '" target="_blank" rel="noopener">'
                . esc_html( $strong_password_keyword )
                . '</a>';

            $intro_text = sprintf(
            // translators: %s: HTML link to a strong password generator.
        ...
        $items = [
            ...
            'len'   => esc_html__( 'Has a minimum length of 12 characters', 'madquick-ppg' ),
        ];
        ?>
        <div id="mq-pass-notice" class="notice notice-info">
            <p class="mq-notice-title"><strong><?php echo esc_html( $title ); ?></strong></p>
            <p class="mq-notice-desc"><?php echo wp_kses_post( $intro_text ); ?></p>
            <ul id="mq-pass-req" class="mq-pass-req">
                <li data-req="lower"><?php echo esc_html( $items['lower'] ); ?></li>
                <li data-req="upper"><?php echo esc_html( $items['upper'] ); ?></li>
                <li data-req="num"><?php echo esc_html( $items['num'] ); ?></li>
                <li data-req="spec"><?php echo esc_html( $items['spec'] ); ?></li>
                <li data-req="len"><?php echo esc_html( $items['len'] ); ?></li>
            </ul>
        </div>
        <?php
    }
    ...
    public static function enqueue_password_checker() {
        ...
        $base_path = defined( 'MADQUICK_PPG_PATH' ) ? MADQUICK_PPG_PATH : plugin_dir_path( __FILE__ );
        $base_url  = defined( 'MADQUICK_PPG_URL' )  ? MADQUICK_PPG_URL  : plugin_dir_url( __FILE__ );
        ...
        wp_localize_script(
            ...
            'mqStrongPassCfg',
            [
                'minLen' => 12, // keep in sync with UI copy & is_strong_password()
            ]
        );
        ...
        $css_rel    = 'assets/css/strong-pass.css';
